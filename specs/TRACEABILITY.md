...
| S010 Testing, CI, binary, samples | `.github/workflows/ci.yml`, `.github/workflows/release.yml`, `box.json`, `dev/`, `samples/`, `parity.yaml` | strict Composer validation, dependency audit, Pint, covered Pest suite, self-check, release-script tests, sample checks, website build, PHAR version/check/checksum smoke tests | `docs/RELEASE.md`, `samples/README.md` | Implemented and locally verified; remote publication gates run from the tagged release workflow |
