...
**Parity supports Parity per-test reports, Parity JSON, Clover XML, Cobertura XML, and PHPUnit XML.**

- **Parity per-test report directory** (`.parity/per-test/`): Parity-native attribution directory with one normalized report per test file plus an `index.json` manifest.
...
Use `coverage_xml` as a string or array; parity uses the first existing file or directory (with `index.json` or `index.xml` for directories). Put the most detailed report first and the most portable fallback last:
...
|--------|--------|------------------|----------------|--------|
| **Parity per-test** | `.parity/per-test/` | **Yes** | **Yes** | Parity-native directory generated by `parity test`; one normalized report per expected test file. |
...
- **Parity per-test**: If you point `coverage_xml` at a directory containing `index.json`, parity merges one report per test and computes `matched-coverage` and `coverage-attribution` from that data.
...
To get per-test coverage in the table, use an attribution-capable source and list it first in `coverage_xml`:
...
Parity will use the first attribution-capable source when present and show the "Covered by" column.
